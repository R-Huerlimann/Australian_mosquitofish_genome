...
independently by size in each haplotype. Homologous chromosomes may differ in
length, meaning that independently assigned chromosome numbers may not identify
corresponding chromosomes across the two haplotypes.
...
3. determine the one-to-one chromosome correspondence from the alignment;
4. number the chromosomes of the primary haplotype sequentially, typically by
   decreasing length; and
5. assign chromosome numbers to the second haplotype according to their
   correspondence with the primary haplotype.

Chromosome orientation should also be made consistent between haplotypes where
this is supported by the alignment. Chromosome correspondence should therefore
be established from whole-genome alignment and synteny rather than inferred
from scaffold length alone.

In this workflow, whole-genome alignment and synteny assessment were performed using Charles Plessy's [`nf-core/pairgenomealign`](https://github.com/nf-core/pairgenomealign)
Nextflow pipeline, as described in [Step 10 — Whole-Genome Synteny Analysis](03_assembly_and_scaffolding_QC.md#step-10--whole-genome-synteny-analysis).


### Note for the *Gambusia holbrooki* genome version 1

In genome version 1, chromosome-scale scaffolds were numbered independently
within HAP1 and HAP2 according to decreasing scaffold length. Consequently,
matching chromosome numbers do not always identify corresponding chromosomes
between the two haplotypes.

The chromosome sequences and assembly structure are unaffected. The issue is
limited to the numerical identifiers assigned to the HAP2 chromosomes.
Whole-genome alignment identified the following correspondence:
